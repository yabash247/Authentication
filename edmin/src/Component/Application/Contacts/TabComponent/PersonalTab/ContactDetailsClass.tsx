      ...
      {selectedUser ?
        <div className="profile-mail">
          <div className="d-flex">
            <Image className= 'img-100 img-fluid m-r-20 rounded-circle update_img_0' src= {dynamicImage(selectedUser.avatar)} alt= 'users'  />
            <div className="flex-grow-1 mt-0">
              ...
              <UL className='simple-list flex-row' >
                <LI><Link to={Href} className="font-primary" onClick={() => editUsers(selectedUser)}>{Edit}</Link></LI>
                <LI><Link to={Href} className="font-primary" onClick={() => deleteUser(selectedUser.id)}>{Delete}</Link></LI>
                <LI><Link to={Href} className="font-primary" onClick={()=> dispatch(setHistory())}>{History}</Link></LI>
                <LI><Link to={Href} className="font-primary" onClick={() => printModalToggle()}>{Print}</Link></LI>
              </UL>
            </div>
          </div>
