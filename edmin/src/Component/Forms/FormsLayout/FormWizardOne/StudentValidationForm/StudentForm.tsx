...
import { StudentFormPropsType } from "../../../../../Type/Forms/FormsLayout/FormWizardOne";
    ...
    <Form onSubmit={(e) => e.preventDefault()} className="form-wizard">
      ...
      <div>
        <div className="text-end pt-3">
          {level > 1 && (
            <Btn color="secondary" onClick={() => dispatch(handleStudentBackButton())} >
              {Previous}
            </Btn>
          )}
          <Btn color="primary" className={`ms-1 ${level === 4 ? "disabled" : ""}`} onClick={() => dispatch(handleStudentNextButton())} >
            {Next}
          </Btn>
        </div>
      </div>
