...
const ToolTipAndPopover = () => {
  ...
  return (
    <>
      <Btn color="info" className='px-xl-2 px-xxl-3' onClick={tooltipsAndPopoversToggle}>{TooltipsAndPopovers}</Btn>
      <CommonModal isOpen={toolAndPopover} toggle={tooltipsAndPopoversToggle} title={TooltipPopoverModal}>
        <H5>{PopoverModal}</H5>
        <P className="mt-2">{`This `}
          <Btn color="success" className="popover-test" title="this is amazing content" id='TooltipExample1'>{ButtonSmall}</Btn>{` triggers a popover on click.`}
        </P>
